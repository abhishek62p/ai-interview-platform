            ...
            </TabsList>
            {
                ...
                <section className="items-end  flex justify-items-end h-full">
                        <div className="flex items-center self-end justify-between gap-4">
                            {user?.image ? (
                                <Image src={user.image} height={40} width={40} className="rounded-full" alt={user?.name || "user image"} />
                            ) : (
                                <div className="h-[40px] w-[40px] rounded-full bg-[#e5e7eb] flex items-center justify-center border border-[#d1d5db]">
                                    <span className="text-sm font-semibold text-[#111827]">{getInitials(user.name)}</span>
                                </div>
                            )}
                            <SignOutButton />
                        </div>
