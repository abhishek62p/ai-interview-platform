...
export const columns: ColumnDef<MockInterviewType>[] = [
        ...
        cell: ({ row }) => {
            ...
            return (
                <div className="flex flex-col gap-1">
                    ...
                    {interview.isScheduled && interview.scheduledAt && (
                        <div className="text-xs text-gray-500" suppressHydrationWarning>
                            📅 {formatDateTimeUTC(interview.scheduledAt)}
                        </div>
                    )}
                </div>
            );
    ...
    ,
    {
        ...
        enableSorting: true,
        cell: ({ row }) => <div className="lowercase" suppressHydrationWarning>{formatDateUTC(row.getValue<Date>("createdAt"))}</div>,
        size: 100
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            const isScheduled = original.isScheduled;
            const scheduledLabel = isScheduled ? "HR Scheduled" : "Mock";
            const expires = isScheduled && original.expiresAt ? new Date(original.expiresAt) : null;
            return (
                <div className="flex flex-col text-xs leading-tight">
                    <span className="capitalize font-medium">{scheduledLabel}</span>
                    {isScheduled && original.scheduledAt && (
                        <span className="text-[10px] text-gray-500" suppressHydrationWarning>Starts: {formatDateTimeUTC(original.scheduledAt)}</span>
                    )}
                    {expires && (
                        <span className="text-[10px] text-red-600" suppressHydrationWarning>Expires: {formatDateTimeUTC(expires)}</span>
                    )}
                </div>
            );
        },
